# Sera "sourcé" par dubash après que ses fonctions soient définies.# Fallbacks si lancé hors-ordre
...
PYT
  ...
  if [[ "${1:-}" == "--json" ]]; then want_json="true"; fi  python3 - "$API_PORT" "$want_json" <<'PYT'
import sys, json, urllib.request, os
port     = int(sys.argv[1])
want_json = (sys.argv[2].lower() == "true")def fetch(url):
    with urllib.request.urlopen(url) as r:
        return json.loads(r.read().decode("utf-8"))# Chargement des cercles
items = []
try:
    data = fetch(f"http://127.0.0.1:{port}/circles")
    items = data.get("items", data if isinstance(data, list) else [])
    ...
    try:
        ...
        items = []# Normalisation minimale
items = [{"id": c.get("id"), "parent_id": c.get("parent_id"), "title": c.get("title","")} for c in items]children = {}
ids = set()
for c in items:
    ids.add(c["id"])
    children.setdefault(c["parent_id"], []).append(c)roots = [c for c in items if c["parent_id"] is None or c["parent_id"] not in ids]
total = len(items)max_depth = 0
leaf_count = 0
def walk(node, d):
    ...
    if not kids:
        leaf_count += 1
    for k in kids:
        walk(k, d+1)for r in roots:
    walk(r, 1)branchers = [len(children.get(c["id"], [])) for c in items if len(children.get(c["id"], []))>0]
avg_branching = (sum(branchers)/len(branchers)) if branchers else 0.0def height(node):
    kids = children.get(node["id"], [])
    if not kids: return 1
    return 1 + max(height(k) for k in kids)heights = { str(r["id"]): height(r) for r in roots }result = {
    "total_nodes": total,
