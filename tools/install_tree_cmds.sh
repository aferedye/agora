...
# 1) Extra commands (tree:circles, circle:move)
...
# 2) Patch dubash: insérer le hook juste avant le "case ${1:-help} in"
...
awk '
  ...
  /case *"\$\{1:-help\}" *in/ && !done {
    ...
    print "  case \"${1:-}\" in"
    print "    tree:circles|circle:move)"
  ...
  { print }
' dubash > dubash.new && mv dubash.new dubash

chmod +x tools/extra_cmds.sh dubash
echo "✅ Ajout du hook et des commandes extra."
